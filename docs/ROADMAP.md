...
| 🔵 P3 | LiftSelector ratio / CompositeSelector 报告增强 | — | 小幅改进 |
...
def ruleset_coverage_simulation(
    ...
    :param df: 数据集
    :param rules: pandas query 表达式列表
    :param target: 目标变量列名
    :param labels: 规则标签列表，默认 None（自动编号）
    :return: 仿真结果 DataFrame
    """
```
...
#### 8-2 `CompositeFeatureSelector.to_excel()` 增强

增强现有 `to_excel()` 为多 Sheet 输出：汇总 Sheet（各特征筛选轨迹）+ 每步骤独立 Sheet。
...
- GitHub Actions: Python 3.8/3.10/3.12 矩阵测试 + 代码风格检查 + tag 发布
- Sphinx + ReadTheDocs API 文档站
...
| `v0.5.0` | 筛选补强 + CI/CD + API 文档站 | Theme 8 + 9 | 待开发 |
...
v0.3.0 专业增强（5-7 天，可并行）
  ...
  ├── [规则] ruleset_coverage_simulation + rule_effectiveness_tracking + detect_rule_conflicts
  └── 对应测试
...
v0.5.0 筛选 + 工程化（3-5 天）
  ├── LiftSelector ratio + CompositeSelector 报告增强
  ├── CI/CD（GitHub Actions）+ API 文档站（Sphinx + ReadTheDocs）
  └── mypy 类型检查配置
```
...
| 日期 | 版本 | 变更说明 |
|------|------|---------|
| 2026-04-11 | v1 | 初版规划，按功能主题组织 |
| 2026-04-12 | v2 | 逐文件代码审计，修正 Theme A/G 已完成误判 |
| 2026-04-12 | v3 | 代码风格审计 + toad/optbinning/scorecardpipeline 竞品全面对标；新增拒绝推断(P0)、二维分箱(P2)、反事实解释(P2)；提炼代码风格约定；重排优先级 |
