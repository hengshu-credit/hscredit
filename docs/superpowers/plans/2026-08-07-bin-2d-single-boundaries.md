...
- Every outline segment is horizontal or vertical; concave corners contain no diagonal connector.
...
5. Verify all outline segments are horizontal or vertical and the L-shaped concave corner uses a right-angle bend inside its bin.
6. Verify a single final bin still receives one complete inset outline.
7. Verify all five metric axes reuse identical per-bin gids, segments, and distinct colors.
8. Verify missing-bin plots contain one outline collection for every ID in `solution_`.
9. Verify each final bin receives exactly one correctly positioned white-background, black-text index and that all five metric axes reuse the same labels.
10. Run:
...
4. At each original grid vertex, connect aligned endpoints directly. For a concave corner, choose the candidate right-angle bend that remains inside the current bin and emit two orthogonal segments; when four endpoints represent diagonal components, pair endpoints by source cell.
...
2. Fit `OptimalBinning2D` against `examples/hscredit_yyp.xlsx` for the C3 and Qingyun 24 score columns.
3. Save and inspect the generated `bin_2d_plot`.
4. Confirm every metric axis has one collection and one distinct category color per final bin, one white-background black index per bin, all colored points are strictly inset, all corners are orthogonal, shared edges show both colors, and blue axes frames remain intact.
