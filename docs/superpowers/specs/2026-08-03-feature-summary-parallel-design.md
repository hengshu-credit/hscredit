...
- `True`：使用项目已有的 `tqdm.auto` 只显示一条字段进度，包括“已处理字段数 / 总
  字段数”和“最近处理字段”，不显示正在计算的统计指标或内部执行阶段。
- 并行时可能同时处理多个字段，因此界面显示“最近处理字段”，不把单个名称误报为
  唯一正在执行的字段。字段名称过长时截断显示，不改变结果中的真实字段名。
...
进度报告由线程安全的私有 reporter 管理。关闭进度时使用无操作 reporter；开启时各
工作线程只在一个字段的全部适用指标完成后，通过短临界区更新一次计数和“最近处理
字段”名称。进度条由主线程创建和关闭，工作线程不创建额外进度条；整个调用只显示一
条字段进度，并兼容终端与 Jupyter Notebook。模型重要性不单独产生进度项，也不改变
字段总数。
...
- 验证 `show_progress=False` 不产生进度输出；开启后只显示一条进度，最终计数等于
  有效字段总数，并显示最近处理的字段名，不出现统计指标名称。串行和并行结果不受
  进度显示影响。
