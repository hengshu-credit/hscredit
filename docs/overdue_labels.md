...
阈值在生成标签、比较及剔灰前统一规范化。例如 `3.0`、`"3.0"` 与 `3` 都使用数值 `3`，生成 `MOB1>=3`；非整数小数保留原值。
支持标量、列表、元组、NumPy 数组和 pandas Index/Series。组合式报告按首次出现顺序去重阈值；
`distribution_plot`、`bin_overdues_plot` 保留重复阈值及其与逾期字段的一一对应关系，也支持单个字段配单个阈值。
自动特征报告的“变量综合统计”、KS 和好坏客户分布图沿用第一个逾期标签；开启剔灰时，综合统计的样本数及 IV/KS 也使用该标签剔灰后的样本。
各标签分箱表、总体分布及时间分布仍按每个标签独立剔灰。
恢复本次扩展之前保存的模型报告或逾期预测器时，继续使用原有的 `>` 定义。
通过 hsbin / hsreport 生成逾期分箱表、自动特征报告、模型报告、策略报告、效率分析或原始数据逾期图时，
运行摘要中的 `overdue_operator` 记录实际比较符，`label_combinations` 返回规范化后的实际组合。
逾期图摘要按一一对应关系记录；模型报告通过 `label_datasets` 标明使用生成标签的数据集，显式传入外部 `y` 的数据集不列入其中。
传入现成分箱表绘图时，不把调用参数当作原表的标签定义写入摘要。
