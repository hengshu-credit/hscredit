...
from ...binning import OptimalBinning, ChiMergeBinning, QuantileBinning
...
class SingleFeatureRuleMiner(BaseRuleMiner):
    ...
    def __init__(
        ...
        max_n_bins: int = 20,
        min_n_bins: int = 2,
        min_bin_size: Union[float, int] = 0.05,
    ...
    def _analyze_numerical_feature(self, feature: str) -> Tuple[pd.DataFrame, Any]:
        ...
        """
        # 获取有效数据（非空）
        valid_mask = self.X_[feature].notna()
        feature_values = self.X_.loc[valid_mask, feature]
        target_values = self.y_[valid_mask]
        ...
        try:
            # 准备数据
            X_feature = pd.DataFrame({feature: feature_values})
            
            # 拟合分箱器
            binner.fit(X_feature, target_values)
            ...
            if hasattr(binner, 'splits_') and feature in binner.splits_:
                ...
                # 回退到quantile分箱
                thresholds = self._get_quantile_thresholds(feature_values)
        except Exception as e:
            if self.verbose:
        ...
        for i, threshold in enumerate(thresholds):
            ...
            if i < len(thresholds) - 1:
                results.append(self._calculate_metrics(
                    feature, threshold, '>=', feature_values, target_values
                ))
            # <= 方向
            if i > 0:
                results.append(self._calculate_metrics(
    ...
    def _analyze_categorical_feature(self, feature: str) -> pd.DataFrame:
        ...
        try:
            ...
            results.append({
                ...
                'operator': '==',
                'total_samples': total,
                'selected_samples': int(selected_count),
                'hit_rate': selected_count / total,
                'total_bad': int(self.y_.sum()),
                'selected_bad': int(selected_bad),
                'badrate': badrate,
    ...
    def _get_quantile_thresholds(self, feature_values: pd.Series) -> List[float]:
        """使用等频分箱获取阈值.
    ...
    def _calculate_metrics(
        ...
        selected_count = mask.sum()
        selected_bad = target_values[mask].sum()
    ...
    def get_top_rules(
        ...
        :param top_n: 返回规则数量
        :param metric: 排序指标，'lift', 'f1', 'precision', 'recall', 'ks'
        :param feature: 指定特征（None则取所有特征）
        :param min_lift: 最小lift阈值
        :param min_samples: 最小样本数
        :param ascending: 是否升序排列，默认False（降序）
        ...
        # 添加规则描述
        all_rules['rule_description'] = all_rules.apply(
            lambda x: f"{x['feature']} {x['operator']} {x['threshold']}",
            axis=1
        )
        
        return all_rules.head(top_n).reset_index(drop=True)
    ...
    def get_rule_objects(
        ...
        """获取Rule对象列表（兼容hscredit Rule接口）.
        
        :param min_lift: 最小lift阈值
        :param min_samples: 最小样本数
        :return: Rule对象列表
        """
        mined_rules = self.get_rules(min_lift, min_samples)
