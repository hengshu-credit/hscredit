...
try:
    ...
    warnings.warn("OR-Tools 未安装，ORBinning 将不可用。" "请使用 pip install ortools 安装。", ImportWarning)
...
class ORBinning(BaseBinning):
    ...
    **引用**
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not ORTOOLS_AVAILABLE:
            raise ImportError("OR-Tools 未安装，无法使用 ORBinning。" "请使用 pip install ortools 安装。")
    ...
    def _get_candidate_splits(self, X: pd.Series, y: pd.Series) -> List[float]:
        """获取候选分割点.
    ...
    def _or_numerical(self, X: pd.Series, y: pd.Series) -> List[float]:
        ...
        min_samples = self._get_min_samples(getattr(self, "_n_total_samples", n_samples))

        # ====== 如果启用 CP-SAT，使用真正的约束规划求解器 ======
        if self.use_cp_sat and ORTOOLS_AVAILABLE:
            if self.objective in {"iv", "ks", "gini"}:
                return search_candidate_splits(
                    ...
                    max_samples=self._get_max_samples(len(x_sorted)),
                    monotonic=self.monotonic,
                    time_limit=self.time_limit,
                )
            if self.objective != "iv":
                # entropy/chi2/custom 走现有目标感知搜索，不能静默退化为 IV。
                return self._greedy_fallback(x_sorted, y_sorted, candidates)
            return self._cp_sat_solve(
                x_sorted,
                y_sorted,
                candidates.tolist() if hasattr(candidates, "tolist") else list(candidates),
                total_good,
                total_bad,
                min_samples,
            )
        ...
        # ====== 策略1: DP 最优 IV 分割 ======
        dp_splits = self._dp_optimal_splits(candidates, cum_bad, cum_good, total_good, total_bad, min_samples)
        ...
        for splits in [dp_splits, mono_dp_splits, greedy_splits, topk_splits]:
            ...
            score_raw = self._score_splits(x_sorted, y_sorted, total_good, total_bad, splits)
    ...
    def _resolve_monotonic_direction_cp_sat(self, x_sorted: np.ndarray, y_sorted: np.ndarray) -> str:
        """解析单调性方向.
    ...
    def _dp_topk_with_composite(
        ...
        for k in range(K_min, K_max + 1):
            for iv_score, split_indices in beam.get(k, []):
                ...
                score = self._score_splits(x_sorted, y_sorted, total_good, total_bad, splits)
                if score > best_score:
                    best_score = score
                    best_splits = splits
    ...
    def _local_refine_composite(
        ...
        best_score = self._score_splits(x_sorted, y_sorted, total_good, total_bad, current)
    ...
    def _compute_derived_stats(self, bin_stats: List[Dict], total_good: int, total_bad: int) -> List[Dict]:
        """计算衍生统计指标.
    ...
    def _lift_quality_bonus(self, bin_stats: List[Dict], total_good: int, total_bad: int) -> float:
    ...
    def _build_bins_from_splits(self, x_sorted: np.ndarray, splits: List[float]) -> np.ndarray:
        if len(x_sorted) == 0:
            return np.array([], dtype=int)
        return np.searchsorted(np.asarray(sorted(splits), dtype=float), x_sorted, side="right").astype(int)

    def _lift_profile_focus_score(self, bins: np.ndarray, y_sorted: np.ndarray) -> float:
        monotonic = self.monotonic if self.monotonic in ["ascending", "descending", "peak", "valley"] else "descending"
    ...
    def _score_splits(
        ...
        split_pos = [0] + [int(np.searchsorted(x_sorted, s, side="right")) for s in sorted(splits)] + [n_samples]
        for i in range(len(split_pos) - 1):
            if split_pos[i + 1] - split_pos[i] < min_samples:
                return -1e18
        # 计算纯目标值（IV/KS/gini等），不含 lift bonus
        raw_objective = self._raw_objective_score(x_sorted, y_sorted, total_good, total_bad, splits)
    ...
    def _raw_objective_score(
        ...
        if self.objective in ("iv", "custom"):
            ...
            return self._calculate_objective_score(x_sorted, y_sorted, total_good, total_bad, splits)

    def _segment_composite_score(self, x_sorted: np.ndarray, y_sorted: np.ndarray, splits: List[float]) -> float:
        bins = self._build_bins_from_splits(x_sorted, splits)
        monotonic = self.monotonic if self.monotonic in ["ascending", "descending", "peak", "valley"] else "descending"
        comp = _composite_binning_quality_components(
            bins=bins,
    ...
    def _search_segment_dp_splits(
        ...
        for depth in range(self.max_n_bins - 1):
            ...
            for _, path, _, _ in states:
                ...
                refined = self._local_refine_splits(x_sorted, y_sorted, total_good, total_bad, splits, candidate_pool)
                final_score = self._score_splits(x_sorted, y_sorted, total_good, total_bad, refined)
        ...
        for _depth in range(self.max_n_bins - 1):
            ...
            for _, current, last_idx in states:
                for idx in range(last_idx + 1, len(candidate_pool)):
                    ...
                    full_score = self._score_splits(x_sorted, y_sorted, total_good, total_bad, trial)
            ...
            for _, trial, _ in states:
                refined = self._local_refine_splits(x_sorted, y_sorted, total_good, total_bad, trial, candidate_pool)
    ...
    def _greedy_fallback(self, x_sorted: np.ndarray, y_sorted: np.ndarray, candidates: List[float]) -> List[float]:
        """贪心算法备选方案.
        ...
        while len(selected_splits) < self.max_n_bins - 1 and remaining_candidates:
            ...
            for i, candidate in enumerate(remaining_candidates):
                ...
                score = self._calculate_objective_score(x_sorted, y_sorted, total_good, total_bad, test_splits)
    ...
    def _or_categorical(self, X: pd.Series, y: pd.Series) -> List[float]:
    ...
    def max_lift_distance_iv(lift_weight: float = 0.1, iv_weight: float = 1.0):
        ...
        **原理**
        ...
        def objective(bin_stats: List[Dict], total_good: int, total_bad: int) -> float:
            ...
            # 最大LIFT离1的距离
            max_lift_dist = max(abs(lift - 1.0) for lift in lift_values)
    ...
    def min_lift_distance_iv(lift_weight: float = 0.1, iv_weight: float = 1.0):
        ...
        **原理**
        ...
        def objective(bin_stats: List[Dict], total_good: int, total_bad: int) -> float:
            ...
            # 最小LIFT离1的距离
            min_lift_dist = min(abs(lift - 1.0) for lift in lift_values)
    ...
    def lift_distance_sum_iv(lift_weight: float = 0.1, iv_weight: float = 1.0):
        ...
        **原理**
        ...
        def objective(bin_stats: List[Dict], total_good: int, total_bad: int) -> float:
            ...
            # 最大LIFT距离 + 最小LIFT距离
            lift_distances = [abs(lift - 1.0) for lift in lift_values]
