...
class RuleCondition:
    ...
    def to_expression(self) -> str:
        """转换为表达式字符串."""
        feature_expr = self.feature
        if not str(self.feature).isidentifier():
            feature_expr = f"`{self.feature}`"

        if isinstance(self.threshold, str):
            threshold_expr = repr(self.threshold)
        else:
            threshold_expr = repr(self.threshold)

        return f"{feature_expr} {self.operator} {threshold_expr}"
