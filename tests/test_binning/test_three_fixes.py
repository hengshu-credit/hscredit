...
# ============================================================
# Issue 1: 切分点中的 np.nan 精确表达缺失值普通箱位置
# ============================================================


class TestCutPointsMissingValue:
    """测试切分点输出是否遵循 user_splits 的缺失值位置语义."""

    def test_separate_missing_does_not_add_nan_to_numerical_splits(self, sample_data):
        """独立 -1 缺失箱不能再向数值 user_splits 规则追加歧义 NaN."""
        ...
        assert len(splits) >= 1
        assert not any(pd.isna(value) for value in splits)
        assert splits == sorted(splits), "切分点应升序"

    def test_get_splits_keeps_separate_missing_out_of_rule(self, sample_data):
        """get_splits 同样不能把独立缺失箱编码成普通箱位置."""
        binner = OptimalBinning(method="quantile", max_n_bins=5, missing_separate=True)
        binner.fit(sample_data[["age"]], sample_data["target"])
    ...
    def test_export_rules_keeps_separate_missing_out_of_rule(self, sample_data):
        """export_rules 只导出切点，不用 NaN 表达独立缺失箱."""
        binner = OptimalBinning(method="quantile", max_n_bins=5, missing_separate=True)
        binner.fit(sample_data[["age"]], sample_data["target"])
    ...
    def test_missing_separate_false_exports_learned_ordinary_target(self, sample_data):
        """存在训练缺失值时，应导出学习所得普通箱位置."""
        binner = OptimalBinning(method="quantile", max_n_bins=5, missing_separate=False)
        binner.fit(sample_data[["age"]], sample_data["target"])

        splits = binner["age"]
        assert sum(pd.isna(value) for value in splits) == 1
    ...
    def test_import_rules_strips_nan(self):
        ...
        assert not any(np.isnan(s) for s in internal), f"内部 splits_ 不应含 nan: {internal}"
...
class TestMaxNBinsEnforcement:
    ...
    def test_max_n_bins_respected(self, method):
        ...
        assert n_bins <= max_bins, f"方法 {method}: n_bins={n_bins} > max_n_bins={max_bins}"
    ...
    def test_max_n_bins_with_missing(self, sample_data):
        ...
        assert n_bins <= max_bins, f"正常箱数 {n_bins} > max_n_bins {max_bins}"
    ...
    def test_max_n_bins_with_many_unique_values(self):
        ...
        for max_bins in [3, 5, 8]:
            ...
            n_bins = binner.n_bins_.get("feat", 0)
            assert n_bins <= max_bins, f"max_n_bins={max_bins}: 实际 {n_bins} 箱"
...
class TestBinLabelsFormat:
    ...
    def test_labels_left_closed_right_open(self, sample_data):
        ...
        for label in labels:
            if label in ("missing", "special"):
                continue
            # 应以 '[' 开头, 以 ')' 结尾
            assert label.startswith("["), f"标签 '{label}' 应以 '[' 开头 (左闭)"
            assert label.endswith(")"), f"标签 '{label}' 应以 ')' 结尾 (右开)"
    ...
    def test_first_bin_starts_with_neg_inf(self, sample_data):
        ...
        assert valid_labels[0].startswith("[-inf,"), f"第一箱应以 '[-inf,' 开头, 实际: {valid_labels[0]}"
    ...
    def test_last_bin_ends_with_pos_inf(self, sample_data):
        ...
        table = binner.get_bin_table("age")
        valid_labels = [l for l in table["分箱标签"] if l not in ("missing", "special")]
        assert valid_labels[-1].endswith("+inf)"), f"最后箱应以 '+inf)' 结尾, 实际: {valid_labels[-1]}"
    ...
    def test_digitize_consistency(self, sample_data):
        ...
        assert bin_idx == 1, f"值 {cut_value} 应归入 bin 1 (左闭), 实际 bin {bin_idx}"
    ...
    def test_all_methods_use_left_closed_labels(self, sample_data, method):
        ...
        for label in table["分箱标签"]:
            if label in ("missing", "special"):
                continue
            assert label.startswith("[") and label.endswith(")"), f"方法 {method}: 标签 '{label}' 不是 [a, b) 格式"
...
class TestIntegration:
    ...
    def test_full_workflow(self, sample_data):
        ...
        # 1. 独立缺失箱不污染普通切分点规则
        cuts = binner["age"]
        assert not any(pd.isna(value) for value in cuts)
        ...
        for label in table["分箱标签"]:
            if label in ("missing", "special"):
                continue
            assert label.startswith("[") and label.endswith(")"), f"标签 '{label}' 不是 [a, b) 格式"

        # 4. 导出 → 导入往返正确
        rules = binner.export_rules()
        binner2 = OptimalBinning(missing_separate=True)
        binner2.import_rules(rules)
        np.testing.assert_array_almost_equal(binner.splits_["age"], binner2.splits_["age"])
