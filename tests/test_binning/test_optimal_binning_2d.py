...
class TestOptimalBinning2DPlot:
    ...
    def test_plot_axis_labels_follow_bin_indices_when_bin_table_is_shuffled(self, sample_df):
        ...
        binner.fit(sample_df, y=sample_df["target"], features=["age", "income"])
        expected_x = [
            binner.binner_x_.bin_tables_["age"].set_index("分箱").loc[i, "分箱标签"] for i in range(binner.n_bins_x_)
        ]
        expected_y = [
            binner.binner_y_.bin_tables_["income"].set_index("分箱").loc[i, "分箱标签"] for i in range(binner.n_bins_y_)
        ]
...
class TestOptimalBinning2DCustom:
    ...
    def test_user_splits(self, sample_df):
        """测试用户自定义分箱规则."""
        binner = OptimalBinning2D(user_splits_x=[25, 35, 45], user_splits_y=[5000, 10000, 20000])
        binner.fit(sample_df, y=sample_df["target"], features=["age", "income"])
    ...
    def test_user_splits_nan_maps_missing_to_selected_ordinary_bin(self, sample_df):
        """用户规则中的 np.nan 应选择普通箱，而不是预留独立缺失箱."""
        binner = OptimalBinning2D(
            user_splits_x=[25, 35, 45],
        ...
        assert not missing_y.any()
        assert binner.binner_y_._user_missing_bin_targets_["income"] == 3
        assert binner.solution_.shape == (binner.n_bins_x_, binner.n_bins_y_)

        table = binner.get_bin_table()
        missing_bins = table["分箱标签"].str.endswith(" × 缺失值")
        assert not missing_bins.any()
    ...
    def test_axis_missing_separate_overrides_global(
        ...
        assert any(pd.isna(value) for value in rules["age"]) is (not expected_x)
        assert any(pd.isna(value) for value in rules["income"]) is (not expected_y)
...
class TestOptimalBinning2DExport:
    ...
    def test_export_rules(self, sample_df):
        """缺失独立箱由配置表达，导出规则不再用 NaN 冒充普通箱位置."""
        ...
        assert not any(pd.isna(value) for value in rules["age"])
        assert not any(pd.isna(value) for value in rules["income"])

    def test_import_rules(self, sample_df):
        """导入规则必须重拟合轴分箱并立即改变 transform 使用的状态."""
        binner = OptimalBinning2D(max_n_bins=4)
        binner.fit(sample_df, y=sample_df["target"], features=["age", "income"])
        ...
        assert binner.user_splits_x[1:] == [25, 35]
        assert binner.user_splits_y == [5000, 10000]
        assert binner.user_splits_fixed_x is True
        assert binner.user_splits_fixed_y is True
        assert binner.splits_x_.tolist() == [25.0, 35.0]
        assert binner.splits_y_.tolist() == [5000.0, 10000.0]
        assert not before.equals(after)

    def test_missing_without_merge_target_remains_separate_in_2d(self):
        """missing_separate=False 且不存在普通箱时，二维分箱不能把 -1 强制改成 0."""
        df = pd.DataFrame(
            {
        ...
        binner = OptimalBinning2D(max_n_bins=2, missing_separate=False)

        binner.fit(df, y=df["target"], features=["x", "y"])

        assert binner.binner_x_.transform(df[["x"]]).iloc[:, 0].eq(-1).all()
        assert binner._has_missing_x_ is True
    ...
    def test_get_bin_table_cross(self, sample_df):
        ...
        assert list(table.columns[:6]) == ["指标名称", "指标含义", "分箱", "分箱标签", "样本总数", "样本占比"]
...
class TestOptimalBinning2DMerge:
    ...
    def test_missing_row_and_column_follow_non_missing_axis_monotonicity(self, sample_df):
        ...
        binner.fit(df, y=df["target"], features=["age", "income"])
        table = binner.get_bin_table().set_index("分箱")
        counts = {int(bin_id): (float(row["坏样本数"]), float(row["好样本数"])) for bin_id, row in table.iterrows()}
        assert not binner._monotonic_violations(binner.solution_, counts, "ascending", "descending")
    ...
    def test_axis_monotonic_constraint(self):
        ...
        counts = {int(row["分箱"]): (float(row["坏样本数"]), float(row["好样本数"])) for _, row in normal.iterrows()}
        assert not binner._monotonic_violations(binner.solution_, counts, "ascending", "ascending")
    ...
    def test_monotonic_hard_constraint_on_noisy_data(self):
        ...
        for trend_x, trend_y in [("ascending", "ascending"), ("descending", "descending"), ("ascending", "descending")]:
            binner = OptimalBinning2D(
                max_n_bins=5,
                max_n_bins_2d=8,
                monotonic_x=trend_x,
