...
def test_resolve_n_jobs_minus_one_reserves_one_cpu():
    with patch("hscredit.utils.parallel.os.cpu_count", return_value=8):
...
def test_resolve_n_jobs_handles_unknown_cpu_count():
    with patch("hscredit.utils.parallel.os.cpu_count", return_value=None):
        assert resolve_n_jobs(-1) == 1
...
def test_risk_model_uses_resolved_cpu_count():
    with patch("hscredit.utils.parallel.os.cpu_count", return_value=12):
        model = RandomForestRiskModel(n_jobs=-1)
    assert model.n_jobs == 11
