...
def plot_tree_pyecharts(
    ...
    **参数**
    ...
    :param tree_obj: ManualTreeExtractor 或 sklearn DecisionTreeClassifier
    :param figsize: 画布尺寸（宽, 高），单位英寸（与 :func:`plot_tree_matplotlib` 同名参数
        对齐）；最终像素 = figsize × dpi。默认 None 时回退到默认画布 1400×900 px
    :param dpi: 每英寸像素数，与 figsize 配合换算画布像素尺寸，默认 100
    :param title: 图表标题
    :param width: 画布宽度（CSS 格式，如 '1400px'）；显式给出时优先于 figsize/dpi
    :param height: 画布高度（CSS 格式）；显式给出时优先于 figsize/dpi
    ...
    **参考样例**
    ...
    # 画布尺寸：显式 width/height（CSS）优先；否则由 figsize×dpi 换算像素；
    # 二者都未给出时回退到默认 1400×900 px（与 :func:`plot_tree_matplotlib` 的
    # figsize/dpi 命名保持一致，便于三种后端统一传参）
    if width is None:
        width = f"{int(figsize[0] * dpi)}px" if figsize else "1400px"
    if height is None:
        height = f"{int(figsize[1] * dpi)}px" if figsize else "900px"
...
def plot_tree_graphviz(
    ...
    样式与实现参考 :func:`plot_tree_matplotlib`：圆形节点徽章 + 主题色标题栏 +
    无表头两列指标表格，全树统一节点宽度（含上限，超长切分条件自动换行），
    人工修改节点（is_manual）使用副主题色边框/标题/徽章，自其向下的连接线
    也统一换为副主题色。卡片采用与 :func:`plot_tree_matplotlib` 一致的直角矩形。
    ...
    **参数**
    ...
    :param tree_obj: ManualTreeExtractor 或 sklearn DecisionTreeClassifier
    :param figsize: 输出图像最大尺寸（宽, 高），单位英寸；graphviz 会在保持纵横比的
        前提下将整张图缩放到不超过该尺寸（与 :func:`plot_tree_matplotlib` 同名参数
        含义对齐）。默认 None 表示按内容自然尺寸输出（不缩放）
    :param dpi: 图像分辨率（每英寸像素数），用于控制位图（png 等）输出的像素大小，
        默认 150。配合 figsize 可灵活控制最终图片大小
    :param save: 保存路径（如 'tree.png' / 'tree.pdf' / 'tree.svg'），渲染格式由文件名
        后缀自动推断；如传入路径中有文件夹不存在，会自动创建，默认 None
    :param title: 图表标题
    ...
    **参考样例**
    ...
    # 构建 DOT 图
    # figsize（英寸）→ graphviz 的 size 属性："w,h"（不加 "!" 时为「最大尺寸」，
    # graphviz 仅在图超出该尺寸时按比例缩小，从而控制最终图片不致过大）；
    # dpi 控制位图分辨率，二者共同决定输出像素尺寸
    size_attr = f' size="{figsize[0]},{figsize[1]}"' if figsize else ""
    dot_lines: List[str] = []
    dot_lines.append('digraph Tree {')
    dot_lines.append(f'    // {title}')
    dot_lines.append(f'    graph [ranksep=0.6, nodesep=0.35, splines=line, bgcolor="#FAFBFF", pad=0.5, dpi={dpi}{size_attr}, concentrate=false];')
    ...
    for node in nodes:
        ...
        # 卡片整体采用直角矩形（不加 STYLE="ROUNDED"），与 plot_tree_matplotlib 的
        # plt.Rectangle 直角节点风格一致（避免「外框圆角、内部矩形直角」的风格冲突）。
        # 徽章（节点编号）改为固定尺寸的圆形（FIXEDSIZE + ROUNDED + 等宽高），
        # 与 plot_tree_matplotlib 的 plt.Circle 圆形徽章一致——并在标题文字右侧追加
        # 一个主题色填充单元格（filler），吸收标题栏的多余宽度，避免徽章被拉宽。
        # 标题栏徽章与条件文本之间的间距用两个不换行空格（&#160;）实现，
        # 不再使用「POINT-SIZE=1 的空格占位单元格」——后者会触发 Pango 的
        # 「pango_cairo_show_layout: assertion 'PANGO_IS_LAYOUT (layout)' failed」告警
        badge_font_pt = TITLE_FONT_PT - 1
        label_html = (
            f'<TABLE BORDER="{border_w}" CELLBORDER="0" CELLSPACING="0" CELLPADDING="0" '
            f'WIDTH="{uniform_w_pt}" COLOR="{border_color}">'
            # 标题栏：徽章（节点编号）+ 切分条件，同一行，主题色背景。
            # 标题栏 TD 显式设置 WIDTH=uniform_w_pt，使其铺满全树统一宽度——
            # 因为 uniform_w_pt 基于 matplotlib 字体测量（偏大），graphviz/Pango 实际
            # 渲染同样文字更窄，若不强制铺满，标题/内容会缩在左侧、右侧留大片空白
            f'<TR><TD WIDTH="{uniform_w_pt}" BGCOLOR="{theme_color}" CELLPADDING="5">'
            f'<TABLE BORDER="0" CELLBORDER="0" CELLSPACING="0" CELLPADDING="0" WIDTH="{uniform_w_pt - 10}">'
            f'<TR>'
            f'<TD WIDTH="{badge_w_pt}" HEIGHT="{badge_w_pt}" FIXEDSIZE="TRUE" ALIGN="CENTER" VALIGN="MIDDLE" BGCOLOR="{badge_bg}" STYLE="ROUNDED">'
            f'<FONT POINT-SIZE="{badge_font_pt}" FACE="Arial" COLOR="{theme_color}"><B>{nid}</B></FONT>'
            f'</TD>'
            ...
            f'</TD>'
            # filler 单元格：主题色，吸收标题栏多余宽度，使徽章保持固定圆形尺寸
            f'<TD BGCOLOR="{theme_color}"></TD>'
            f'</TR>'
            f'</TABLE>'
            f'</TD></TR>'
            # 内容区：渐变色背景 + 无表头两列指标表格。同样显式设置 WIDTH=uniform_w_pt
            # 并 ALIGN=CENTER，使背景铺满整张卡片、指标表格在卡片内水平居中（与
            # plot_tree_matplotlib 的居中两列布局一致）
            f'<TR><TD WIDTH="{uniform_w_pt}" ALIGN="CENTER" BGCOLOR="{fill_color}">'
    ...
    #
    # - 连接端点：用 graphviz 罗盘端口强制「父节点底边中点(:s) → 子节点顶边中点(:n)」，
    #   使所有连接线只从节点的正上方/正下方中央进出（而非从边角或两侧任意位置）
    # - 分支标签位置：标签框始终居中于连线中点，因此通过在标签文字一侧补空格来
    #   让可见符号偏离中点——「<=」(左分支) 右侧补空格 → 符号落在连线中点的左侧；
    #   「>」(右分支) 左侧补空格 → 符号落在连线中点的右侧
    ROOT_ID = 0
    LABEL_PAD = 8  # 分支标签的偏移空格数
    for edge in edges:
        label_text = edge["label"]
        ...
        if label_text == "<=":
            label_str = label_text + " " * LABEL_PAD  # 右侧补空格 → 符号偏左
        else:
            label_str = " " * LABEL_PAD + label_text  # 左侧补空格 → 符号偏右

        dot_lines.append(
            f'    {edge["source"]}:s -> {edge["target"]}:n '
            f'[label="{label_str}", fontcolor="{edge_color}", '
    ...
    if save:
        ...
        if save_dir and not os.path.exists(save_dir):
            os.makedirs(save_dir, exist_ok=True)
        # 渲染格式通过 save 的文件后缀判断（与 plot_tree_matplotlib 的 save 约定一致），
        # 未带后缀时默认 'png'；graphviz 的 render() 会自动在文件名后追加 ".{format}"，
        # 所以这里需要先去掉 save 自带的后缀，否则会生成 "tree.png.png" 这种重复文件
        save_base, ext = os.path.splitext(save)
        fmt = ext[1:].lower() if ext else "png"
        if not save_base:
            save_base = save
        src.render(save_base, format=fmt, cleanup=True)
